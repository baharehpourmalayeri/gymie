...
from app.schema.bookings import WorkoutBookingCreate, CoachBookingCreate
...
def create_workout_booking(booking: WorkoutBookingCreate, user_id: int, db: Session):
    ...
    workout_session = db.query(WorkoutSession).filter(
        WorkoutSession.id == booking.workout_session_id).first()
...
def create_coach_booking(booking: CoachBookingCreate, user_id: int, db: Session):
    ...
    coach_session = db.query(CoachSession).filter(
        CoachSession.id == booking.coach_session_id).first()
...
def get_user_coach_bookings(user_id: int, db: Session):
    bookings = db.query(Booking).filter(
        Booking.user_id == user_id,
        Booking.coach_session_id != None).all()
    return bookings
